  ...
  async function fetchAvailableTimes() {
    ...
    const BUFFER_MINUTES = 10; // Your new buffer
    ...
    // Logic: Filter out slots that conflict with existing bookings
    const freeTimes = ALL_TIME_SLOTS.filter((time12h) => {
      ...
      // Calculate when this specific booking would end
      const proposedEndMinutes = startMinutes + serviceDuration + BUFFER_MINUTES;

      // Check if this time window overlaps with ANY existing booking
      const hasConflict = data.some((booking) => {
        ...
        // Use the duration from the database + buffer for the existing booking
  ...
  fetchAvailableTimes();
}, [appointmentDate, selectedServiceId]); // Re-run if date or service changes
